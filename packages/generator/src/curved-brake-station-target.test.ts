...
describe("curved brake and terminal station target packet", () => {
  ...
  it("curved brake preserves seventh-order seam-friendly geometry, rotates tangent ~pi and displaces ~2r while keeping authored length", () => {
    ...
    const radius =
      curved.parameters.length / Math.abs(curved.parameters.angle!);
    ...
    const reconstructedSpans = generated.file.solvedSpans.map(
      reconstructSolvedSpan,
    );
    const compiled = compileTrack(reconstructedSpans);
    const storedSum = generated.file.solvedSpans.reduce(
      (sum, s) => sum + s.length,
      0,
    );
    expect(
    ...
    for (const span of generated.file.solvedSpans) {
      ...
      physicalByOwner.set(
        actualOwner,
        (physicalByOwner.get(actualOwner) ?? 0) + span.length,
      );
    }
    for (const zone of zones) {
    ...
    const adjacent = seams.filter(
      (s) =>
        s.seamId === "stall-007#7->brake-008" ||
        s.seamId === "brake-008->magnetic-brakes-009",
    );
    expect(
      adjacent,
      `expected exactly 2 curved-brake seams, got ${seams.map((s) => s.seamId).join(", ")}`,
    ).toHaveLength(2);
  ...
  it("missing angle retains exact straight line and magnetic brake stays straight", () => {
    ...
    expect(Math.abs(builtMagnetic.endPose.position[2] - 110)).toBeLessThan(
      1e-6,
    );
    const dotNoAngle =
  ...
  it("station targetSpeed and brake angle validation rejects out-of-range, zero and non-finite with named range errors", () => {
    expect(() =>
      createElement("station", "s", {
        length: 12,
        targetSpeed: -1 as unknown as number,
      }),
    ).toThrow(/targetSpeed.*0.*120/);
    expect(() =>
      createElement("station", "s", {
        length: 12,
        targetSpeed: 200 as unknown as number,
      }),
    ).toThrow(/targetSpeed/);
    expect(() =>
      createElement("station", "s", {
        length: 12,
        targetSpeed: Number.NaN as unknown as number,
      }),
    ).toThrow(/targetSpeed|finite/);
    expect(() =>
      createElement("station", "s", {
        length: 12,
        targetSpeed: Number.POSITIVE_INFINITY as unknown as number,
      }),
    ).toThrow(/finite/);
    expect(
      createElement("station", "s", { length: 12, targetSpeed: 0 }).parameters
        .targetSpeed,
    ).toBe(0);
    expect(
      createElement("station", "s", { length: 12, targetSpeed: 120 }).parameters
        .targetSpeed,
    ).toBe(120);
    const open = createElement("station", "station-open", {
      length: 12,
      bank: 0,
      closed: false,
    });
    ...
    expect(() =>
      createElement("brake", "b", {
        length: 20,
        targetSpeed: 8,
        angle: 0 as unknown as number,
      }),
    ).toThrow(/angle.*zero|zero.*angle/i);
    expect(() =>
      createElement("brake", "b", {
        length: 20,
        targetSpeed: 8,
        angle: Number.NaN as unknown as number,
      }),
    ...
    expect(() =>
      createElement("brake", "b", {
        length: 20,
        targetSpeed: 8,
        angle: Number.POSITIVE_INFINITY as unknown as number,
      }),
    ).toThrow(/finite|angle/);
    expect(() =>
      createElement("brake", "b", {
    ...
    ).toThrow(/angle.*2π|angle.*between/);
    expect(() =>
      createElement("brake", "b", {
        length: 20,
        targetSpeed: 8,
    ...
    expect(() =>
      createElement("overbankedTurn", "ot", {
        radius: 28,
        angle: 3 * Math.PI,
        bank: 0,
      }),
    ...
    expect(
      createElement("overbankedTurn", "ot2", {
        radius: 28,
        angle: 3 * Math.PI,
        bank: 0,
      }).parameters.angle,
    ...
    expect(() =>
      createElement("overbankedTurn", "ot3", {
        radius: 28,
  ...
  it("parser exact allowlist accepts new keys and rejects invalid values and unknown fields via core validation", () => {
      ...
      elements: [
        {
          ...
          kind: "station",
          type: "station",
          parameters: { length: 160, bank: 0, closed: false, targetSpeed: 0 },
        },
        {
          id: "brake-008",
          kind: "brake",
          type: "brake",
          parameters: { length: 220, targetSpeed: 8, bank: 0, angle: Math.PI },
        },
      ],
      gates: [],
      targets: [],
    ...
    const serialized = serializeDesignIntentV1(
      ...
      elements: [
        {
          ...
          kind: "brake",
          type: "brake",
          parameters: { length: 20, targetSpeed: 8, bank: 0, angle: 0 },
        },
      ],
    };
    expect(() =>
      createDesignIntentV1(
        invalidBrakeZero as unknown as Parameters<
          typeof createDesignIntentV1
        >[0],
      ),
    ).toThrow(/angle/);
    expect(() => parseDesignIntentV1(JSON.stringify(invalidBrakeZero))).toThrow(
      /angle/,
    );
    expect(() =>
      parseDesignIntentV1(
      ...
      elements: [
        {
          ...
          kind: "overbankedTurn",
          type: "overbankedTurn",
          parameters: { radius: 28, angle: 3 * Math.PI, bank: 0.6 },
        },
      ],
    };
    expect(() =>
      parseDesignIntentV1(JSON.stringify(overbankedValid)),
    ).not.toThrow();
    expect(() =>
      parseDesignIntentV1(
        JSON.stringify({
  ...
  it("default file round-trips, preserves authored lengths and operation targets with physical lengths", () => {
    ...
    const magnetic = result.elements.find(
      (e) => e.id === "magnetic-brakes-009",
    )!;
    ...
    const reconstructedSpans = result.file.solvedSpans.map(
      reconstructSolvedSpan,
    );
    const compiled = compileTrack(reconstructedSpans);
    const storedSum = result.file.solvedSpans.reduce(
      (sum, s) => sum + s.length,
      0,
    );
    expect(
      Math.abs(storedSum - compiled.totalLength),
      `stored sum vs compiled totalLength`,
    ).toBeLessThan(1e-9);
    ...
    for (const zone of [brakeZone, magneticZone, stationZone]) {
      ...
      expect(
        Math.abs(width - expected!),
        `zone ${zone.id} width ${width} vs physical ${expected}`,
      ).toBeLessThan(1e-9);
    }
    ...
    const reloadedBrake = loaded.file.intent.elements.find(
      (e) => e.id === "brake-008",
    )!;
    ...
    const loadedReconstructed = loaded.file.solvedSpans.map(
      reconstructSolvedSpan,
    );
    const loadedCompiled = compileTrack(loadedReconstructed);
    const loadedSum = loaded.file.solvedSpans.reduce(
      (s, sp) => s + sp.length,
      0,
    );
    ...
    for (const zone of operationZonesFromCoasterFile(loaded.file)) {
      expect(zone.endDistanceM).toBeLessThanOrEqual(
        loadedCompiled.totalLength + 1e-9,
      );
    }
  });

  it(
    "local regeneration preserves curved brake angle, station target and physical lengths",
    { timeout: 90_000 },
    () => {
      const result = defaultGeneration;
      const beforeBrake = result.elements.find((e) => e.id === "brake-008")!;
      const beforeStation = result.elements.find(
        (e) => e.id === "station-010",
      )!;
      expect((beforeBrake.parameters as BrakeParameters).angle).toBe(Math.PI);
      expect((beforeStation.parameters as StationParameters).targetSpeed).toBe(
        0,
      );
      ...
      const afterBrake = local.generation.elements.find(
        (e) => e.id === "brake-008",
      )!;
      const afterStation = local.generation.elements.find(
        (e) => e.id === "station-010",
      )!;
      expect((afterBrake.parameters as BrakeParameters).angle).toBe(Math.PI);
      expect((afterStation.parameters as StationParameters).targetSpeed).toBe(
        0,
      );
      ...
      const localReconstructed = local.generation.file.solvedSpans.map(
        reconstructSolvedSpan,
      );
      const localCompiled = compileTrack(localReconstructed);
      const localSum = local.generation.file.solvedSpans.reduce(
        (s, sp) => s + sp.length,
        0,
      );
      ...
      const fileBrake = fileLocal.generation.elements.find(
        (e) => e.id === "brake-008",
      )!;
      ...
      const fileLocalReconstructed = fileLocal.generation.file.solvedSpans.map(
        reconstructSolvedSpan,
      );
      const fileLocalCompiled = compileTrack(fileLocalReconstructed);
      const fileLocalSum = fileLocal.generation.file.solvedSpans.reduce(
        (s, sp) => s + sp.length,
        0,
      );
      expect(
        Math.abs(fileLocalSum - fileLocalCompiled.totalLength),
      ).toBeLessThan(1e-9);
      const fileStation = fileLocal.generation.elements.find(
        (e) => e.id === "station-010",
      )!;
